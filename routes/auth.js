...
//       return res.redirect(`https://backend-com.up.railway.app:${pending.cli_port}/callback?${params}`)
//     }
...
//       return res.redirect(`https://backend-com.up.railway.app:${pending.cli_port}/callback?error=${encodeURIComponent(err.message)}`)
//     }
...
authRouter.get('/github/callback', async (req, res) => {
  ...
  try {
    ...
    // Web flow — pass tokens via URL, frontend stores them
    const params = new URLSearchParams({
      access_token: accessToken,
      refresh_token: refreshToken,
    })
    ...
    console.error('[auth/callback]', err)
    if (pending?.isCliFlow && pending?.cli_port) {
      return res.redirect(`http://localhost:${pending.cli_port}/callback?error=${encodeURIComponent(err.message)}`)
    }
    res.status(500).json({ status: 'error', message: 'Authentication failed' })
  }
